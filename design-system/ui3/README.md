...
- Framework-neutral reusable primitives: layout, Button/IconButton, Badge, Avatar, Chit/Chip, Input/Combo/Numeric/Color, Dropdown/Menu, Checkbox/Radio/Switch, Segmented/Tabs, Slider, Modal, Tooltip, Notification/Visual Bell, Comment/Thread composer и Progress; все основные UI3 axes передаются через typed options и `data-*`.
...
Реализация primitives: [src/ui3-primitives.ts](src/ui3-primitives.ts), [src/ui3-primitives.css](src/ui3-primitives.css), подробный API: [docs/UI3_PRIMITIVES.md](docs/UI3_PRIMITIVES.md).
...
6. Подключайте компонентный слой из UI entry:
...
Factories возвращают `{ root, destroy() }`; dropdown/modal дополнительно дают `open()`, `close()` и `toggle()`. Примитивы не требуют React/Vue и используют native DOM + ARIA, поэтому их можно обернуть адаптером любого UI-фреймворка.
...
Снимок получен 31 июля 2026 года, повторно проверен 2 августа и актуально сверён 6 августа 2026 года напрямую по [UI3 — Figma’s UI Kit (Community)](https://www.figma.com/design/C87oqghCfxdKR5iYmCxEVP/UI3--Figma-s-UI-Kit--Community-?node-id=2012-163499). Проверены все 29 страниц, включая компонентные рецепты и страницу Icons. Это полный набор локальных foundations именно этой публичной копии. Подробная сверка и решение по замене материалов: [docs/FIGMA_SOURCE_AUDIT.md](docs/FIGMA_SOURCE_AUDIT.md). Он не является обещанием совпадения с закрытой текущей системой, которую Figma использует в production.

Каталог правил и variant axes для всех страниц: `references/ui3-community-pages.md` в Codex skill. Runtime-пакет содержит foundations, CSS-адаптер и SVG-иконки; готовые product-компоненты нужно реализовывать в стеке конкретного плагина и проверять визуально.
