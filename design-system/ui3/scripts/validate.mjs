...
const indexCss = await text("src/index.css");
const primitivesCss = await text("src/ui3-primitives.css");
const primitivesTs = await text("src/ui3-primitives.ts");
...
assert(indexCss.includes('@import "./ui3-primitives.css";'), "src/index.css does not expose the UI3 primitives stylesheet.");
for (const contract of [
  ...
  ".ui3-tooltip",
  ".ui3-notice",
  ".ui3-comment-composer",
  ".ui3-progress",
  ":focus-visible",
  "[aria-disabled",
  "[aria-selected",
]) {
  assert(primitivesCss.includes(contract), "UI3 primitives CSS is missing contract " + contract + ".");
}
for (const factory of [
  ...
  assert(primitivesTs.includes(`function ${factory}`) || primitivesTs.includes(`export function ${factory}`), "UI3 primitives TypeScript is missing factory " + factory + ".");
}
assert(primitivesTs.includes("export const ui3Primitives"), "UI3 primitives TypeScript is missing the aggregate export.");
assert(!/#[0-9a-f]{3,8}\b/i.test(primitivesCss), "UI3 primitives CSS contains a hard-coded color.");
for (const contract of [
  "--ui3-dropdown-bg",
  "--ui3-dropdown-text",
  "select.ui3-dropdown",
