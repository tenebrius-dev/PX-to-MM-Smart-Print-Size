...
В комплекте уже есть framework-neutral DOM layer в `src/ui3-primitives.ts` и `src/ui3-primitives.css`. Он покрывает:

1. Button и IconButton: default, hover, pressed, focus-visible, disabled, danger, success, FigJam, inverse, link.
2. Input/Textarea, Numeric, Color и Combo input: default, hover, focus, invalid, disabled, multiline и variable-friendly states.
3. Checkbox, Radio, Switch, SegmentedControl, Tabs и Slider.
4. Menu/Dropdown, Tooltip и локальные SVG icon helpers; варианты menu rows передаются через `Ui3MenuOptions` и `Ui3MenuItem`.
5. Dialog/Modal, Notification, Visual Bell, Comment/Thread, Comment composer и Progress.
6. Badge, Avatar, Chit/Chip и layout primitives: Stack, Row, Divider, Surface, ScrollArea.
7. Variant axes Figma (`stroke`, `iconLead`, `variable`, `dropdown`, `ghost`, `state`, `color`) доступны typed options и сохраняются в `data-*` для adapter-слоя.

Использование:

~~~ts
import "./design-system/ui3/src/index.css";
import { createButton } from "./design-system/ui3/src/ui3-primitives";

const button = createButton({ label: "Import", variant: "primary" });
document.querySelector("#actions")?.append(button.root);
~~~

Полная таблица API и граница переноса: [UI3_PRIMITIVES.md](UI3_PRIMITIVES.md).
